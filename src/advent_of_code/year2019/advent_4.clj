...
(defn digits
  ([value] (reverse (digits value [])))
  ([value acc]
   (if (zero? value)
     acc
     (digits (quot value 10) (conj acc (mod value 10))))))

(defn is-not-decrease
  [digits]
  (if (== 1 (count digits))
    true
    (if (<= (first digits) (second digits))
      (is-not-decrease (rest digits))
      false)))

(defn is-only-two-adjacent-same
  ([digits] (is-only-two-adjacent-same
              (rest digits)
              (first digits)
              1))
  ([digits current-number count]
   (if (empty? digits)
     (== 2 count)
     (if (== (first digits) current-number)
       (is-only-two-adjacent-same
         (rest digits)
         current-number
         (inc count))
       (if (== 2 count)
...
(defn is-valid
  ...
  (let [digits (digits value)]
    (and (is-only-two-adjacent-same digits)
         (is-not-decrease digits))))

(count (filter true? (map is-valid my-range)))
